...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_2__landlord__ = __webpack_require__(295);
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_4__ionic_native_file_path__ = __webpack_require__(193);
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
...
var LandlordPage = /** @class */ (function () {
    function LandlordPage(navCtrl, navParams, toastCtrl, camera, filePath, f, loadingCtrl) {
        ...
        firebase.database().ref('/Bookings_TBL/').on('value', function (snapshot) {
            snapshot.forEach(function (snap) {
                _this.bookings.push({ address: snap.val().address, date: snap.val().date, flatName: snap.val().flatName, tenantID: snap.val().tenantID, landLordContactNo: snap.val().landLordContactNo, landlordEmail: snap.val().landlordEmail, landlordName: snap.val().landlordName, landlordSurname: snap.val().landlordSurname, landID: snap.val().landID, time: snap.val().time, clientName: snap.val().clientName, clientSurname: snap.val().clientSurname, clientContact: snap.val().clientContact, clientEmail: snap.val().clientEmail, clientID: snap.val().clientID });
